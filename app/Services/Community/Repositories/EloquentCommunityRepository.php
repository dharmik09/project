...
class EloquentCommunityRepository extends EloquentBaseRepository implements CommunityRepository {
    ...
    public function saveConnectionRequest($connectionRequestData, $token)
    {
        if(isset($token) && $token != '' && !empty($token)) {
            ...
            if (isset($availableRequest) && !empty($availableRequest)) {
                ...
                $updateRequest = $this->model->where('id', $availableRequest->id)->update($connectionRequestDetails);
                $return = $this->model->where('id', $availableRequest->id)->first();
            } else {
                $reverseRequest = $this->model->where('tc_receiver_id', $connectionRequestData['tc_sender_id'])->where('tc_sender_id', $connectionRequestData['tc_receiver_id'])->first();
                if (isset($reverseRequest) && !empty($reverseRequest)) {
                    $connectionRequestDetails['tc_status'] = Config::get('constant.CONNECTION_PENDING_STATUS');
                    $updateRequest = $this->model->where('id', $reverseRequest->id)->update($connectionRequestDetails);
                    $return = $this->model->where('id', $reverseRequest->id)->first();
                } else {
    ...
    *  @default : Default passing 2 use as a response to show not connected
    */
    public function checkTeenConnectionStatusForNetworkMemberPage($receiverId, $senderId)
    {
        $flag = [];
        $flag['count'] = 2;
        $availableRequest = $this->model->where('tc_receiver_id', $receiverId)->where('tc_sender_id', $senderId)->first();
        if ($availableRequest && !empty($availableRequest)) {
            ...
            $availableRequestReverse = $this->model->where('tc_receiver_id', $senderId)->where('tc_sender_id', $receiverId)->first();
