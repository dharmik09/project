...
//        }
    ...
    */
     ...
     * return : array/object of the activities which are attempted by teenager passed and total no of questions
    */
    public function getPointsbyQuestion($questionId){
            $result = DB::table(config::get('databaseconstants.TBL_LEVEL2_ACTIVITY'))->where("id", $questionId)->where("deleted", 1)->first();
            return $result;  
    }
    ...
    */
    public function getAllNotAttemptedActivitiesBySection($teenagerId,$section)
    {
        ...
        foreach($activities as $key => $activity)
        {
            ...
            $activities[$key]->l2ac_image = ($activity->l2ac_image != "") ? Storage::url(Config::get('constant.LEVEL2_ACTIVITY_ORIGINAL_IMAGE_UPLOAD_PATH').$activity->l2ac_image) : Storage::url(Config::get('constant.LEVEL2_ACTIVITY_ORIGINAL_IMAGE_UPLOAD_PATH')."proteen-logo.png");
            $activities[$key]->options = $optionsWithId;
        }
