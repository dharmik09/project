...
class Level2Activity extends Model
{
    ...
    public function getActiveLevel2Activity($id)
    {
        $level2activities = DB::select( DB::raw("SELECT
                                              activity.*, apptitude.apt_name, mi.mit_name, interest.it_name, personality.pt_name, GROUP_CONCAT(options.l2op_option) AS l2op_option, GROUP_CONCAT(options.l2op_fraction) AS l2op_fraction FROM 
