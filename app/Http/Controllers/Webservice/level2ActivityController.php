...
class level2ActivityController extends Controller {
    ...
    public function getLevel2Activity(Request $request) {
        ...
        if($request->userId != "" && $teenager) {
            ...
            $response['is_section_4'] = 0;
            if ($teenager->t_school > 0 && $teenager->t_school_status == 1) {
                if (count($data['section_4']) > 0 || number_format((float)$section4Percentage, 0, '.', '') == 100) {
                    $response['is_section_4'] = 1;      
                } 
            }
            $response['status'] = 1;
            $response['login'] = 1;
        ...
        if($request->userId != "" && $request->activityID != "" && $request->optionId != "" && $request->timer != "" && $teenager) {
            ...
            if (isset($request->userId) && $request->userId > 0 && isset($answers['timer']) && $answers['timer'] != '' && isset($answerID) && isset($questionID) && $questionID != 0 && $answerID != 0) {
                ...
                if($questionsArray){
                    ...
                    if($section2Percentage == 100){
                        $dispatchJob = Helpers::professionMatchScaleCalculate($section2Collection, $request->userId);
                    }
