...
class SignupController extends Controller
{
    ...
    public function doSignup(TeenagerSignupRequest $request) {
        ...
        if($teenagerDetail['t_email'] != '' && $teenagerDetail['t_social_provider'] == 'Normal'){
            $parsed = Helpers::get_string_between($teenagerDetail['t_email'], '@', '.');
            if($parsed == 'gmail' || $parsed == 'outlook' || $parsed == 'hotmail' || $parsed == 'inbox'){
            ...
            if (isset($teenagerDetail['t_social_provider']) && $teenagerDetail['t_social_provider'] != '' && $teenagerDetail['t_social_provider'] == 'Normal') {
                if (($teenagerDetail['t_email'] != '') && $teenagerDetail['password'] != '' && $teenagerDetail['t_name'] != '') {
                    if (!filter_var($body['email'], FILTER_VALIDATE_EMAIL)) {
                        ...
                        // ------------------------end sending mail ----------------------------//
                            $responseMsg = 'Hi <strong>'.$teenagerDetailbyId->t_name.'</strong>, <br/> The access link to activate your account has been sent to your registered eMailID. If you do not receive the confirmation message within a few minutes of signing up, please check your Junk/Spam/Promotions E-mail folder just in case the confirmation email got delivered there instead of your inbox.<strong>'.$teenagerDetailbyId->t_email.'</strong>';
