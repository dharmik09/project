    ...
    <body>
        ...
        <div class="container">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="{{url('school/home')}}"><img src="{{ Storage::url('frontend/images/proteen_logo.png')}}" alt=""></a>
            </div>
            <div class="navbar-collapse collapse navbar-responsive-collapse ">
                <ul class="nav nav_basic navbar-nav navbar-right home_page_navigation non_teen">
                    <li class="{{(Route::getFacadeRoot()->current()->uri() == 'school/update-profile')? 'active':''}}"><a href="{{url('/school/update-profile')}}">My Profile</a></li>
                    <li class="{{(Route::getFacadeRoot()->current()->uri() == 'school/home')? 'active':''}}"><a href="{{url('/school/home')}}">My Student</a></li>
                    <li class="{{(Route::getFacadeRoot()->current()->uri() == 'school/progress')? 'active':''}}"><a href="{{url('/school/progress')}}">Progress</a></li>
                    <li class="{{(Route::getFacadeRoot()->current()->uri() == 'school/get-gift-coins' || Route::getFacadeRoot()->current()->uri() == 'school/history')? 'active':''}}"><a href="{{ url('school/get-gift-coins') }}">My ProCoins </a></li>
                    <li class="user_avatar">
                        <a href="#" class="drop_down_menu">
                           <span class="user_detail_name">{{Auth::guard('school')->user()->sc_first_name}}</span>
                            <?php
                             $photo = Auth::guard('school')->user()->sc_photo;
                             
                             if ($photo != '' && isset($photo)) {
                                $profilePicUrl = Storage::url(Config::get('constant.CONTACT_PERSON_ORIGINAL_IMAGE_UPLOAD_PATH') . $photo);
                               } else {
                                $profilePicUrl = Storage::url(Config::get('constant.CONTACT_PERSON_THUMB_IMAGE_UPLOAD_PATH') . "proteen-logo.png");
                               }                                                            
                              ?>
                        ...
                        <ul class="navigation_prime menu_dropdown" style="display: none;">
                            ...
                            <li>
                                <a href="{{url('/school/logout')}}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">Log Out</a>
                            </li>
                        </ul>
                    </li>
                </ul>
