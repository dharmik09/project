...
<section class="content-header">
    <h1>
        {{(isset($parentDetail) && !empty($parentDetail)) ? $parentDetail->p_first_name : ''}} -
...
<section class="content">
    <div class="row">
        ...
        <div class="col-md-9">
            <a class="btn btn-danger btn-flat pull-right" href="{{ url('admin/level4AdvanceActivityParentTask') }}">Back</a>
        </div>
        <div class="box-body">
        </div>
        <div class="col-md-12">
                                ...
                                <tr class="header_text">
                                  ...
                                  <td style="vertical-align: top;">
                                      <div class="col-md-12">
                                            ...
                                            <div class="col-md-2">
                                                Point&nbsp;&nbsp;
                                                <input type="number"  value="{{(isset($task->l4aapa_earned_points) && !empty($task->l4aapa_earned_points)) ? $task->l4aapa_earned_points : 0}}" class="form-control number_btn" name="boosterPoint[{{$task->id}}]">
                                            </div>
                                            <div class="col-md-3">
                                                <span style="vertical-align: middle;">Status&nbsp;&nbsp;</span>
                                              <span class="verify_reject"><input type="radio" value="2" <?php if(isset($task->l4aapa_is_verified) && $task->l4aapa_is_verified == 2){echo "checked";}  ?>  name="status[{{$task->id}}]" id='verify_{{$task->id}}'><label for='verify_{{$task->id}}'>Verify</label></span>
                                              <span class="verify_reject"><input type="radio" value="3" <?php if(isset($task->l4aapa_is_verified) && $task->l4aapa_is_verified == 3){echo "checked";}  ?> name="status[{{$task->id}}]" id='reject_{{$task->id}}'><label for='reject_{{$task->id}}'>Reject</label></span>
                                            </div>
                                            <input type="hidden" name="verified_status[{{$task->id}}]" value="{{$task->l4aapa_is_verified}}"/>
                                            <div class="col-md-3">
                                      ...
                                      <div class="tab_detail_cont_text_area">
                                          <textarea placeholder="Enter note here" name="note[{{$task->id}}]">{{(isset($task->l4aapa_note) && !empty($task->l4aapa_note)) ? $task->l4aapa_note : ''}}</textarea>
                                      </div>
                                  </td>
                    ...
                    <div role="tabpanel" id="document_tab" class="tab-pane fade {{(isset($typeId) && $typeId == 2)?'in active':''}}">
                        @if(isset($userAllDocumentTasks) && !empty($userAllDocumentTasks))
                            ...
                            <div class="box-footer">
                                <?php
                                    $disable = '';
                                    if ($userAllDocumentTasks[0]->l4aapa_is_verified != 1) { $disable = 'disabled'; } ?>
                    ...
                    <div role="tabpanel" id="video_tab" class="tab-pane fade {{(isset($typeId) && $typeId == 1)?'in active':''}}">
                       @if(isset($userAllVideoTasks) && !empty($userAllVideoTasks))
                            ...
                            <div class="box-footer">
                                <?php
                                    $disable = '';
                                    if ($userAllVideoTasks[0]->l4aapa_is_verified != 1) { $disable = 'disabled'; } ?>
                                <button type="submit" class="btn btn-primary btn-flat" <?php echo $disable;?>>{{trans('labels.savebtn')}}</button>
                            </div>
                            </form>
                            @else
...
<script type="text/javascript">
   ...
   function deleteLevel4AdvanceTaskParent(media_id,media_name,media_type)
    {
        res = confirm('Are you sure you want to delete this record?');
        if(res){
        $.ajax({
            url: "{{ url('admin/deleteParentAdvanceTask') }}",
            type: 'post',
            data: {
                "_token": '{{ csrf_token() }}',
                "task_id": media_id,
