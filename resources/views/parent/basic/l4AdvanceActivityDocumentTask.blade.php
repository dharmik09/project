...
@if(count($userLevel4AdvanceDocumentTask) >= Config::get('constant.DEFAULT_TOTAL_ADVANCE_DOCUMENT_COUNT'))
    <div class="upload-content">
        <div class="no-data">
            <div class="nodata-middle">
                You reached maximum document upload limit. Total uploaded document is {{ count($userLevel4AdvanceDocumentTask) }}!
            </div>
        </div>
    </div>
@else
...
@endif
<span>You can upload maximum {{ Config::get('constant.DEFAULT_TOTAL_ADVANCE_DOCUMENT_COUNT') }} document.</span>
<?php $pendingTask = 0; ?>
...
<div class="upload-content">
    <ul class="upld-img">
        <form id="advance_task_review" class="advance_task_review" method="post" enctype="multipart/form-data">
            ...
            <?php 
                if(Storage::size($level4AdvanceOriginalImageUploadPath.$task->l4aapa_media_name) > 0 && $task->l4aapa_media_name != '') { 
                    $media =  Storage::url($level4AdvanceOriginalImageUploadPath.'document.png');
                    $mediaPath = Storage::url($level4AdvanceOriginalImageUploadPath.$task->l4aapa_media_name);
                }else{
                    $media =  Storage::url($level4AdvanceOriginalImageUploadPath.'no_document.png');
                ...
                <div class="upd-detail clearfix img-type">
                    ...
                    <div class="detail-cont">
                        ...
                        <div class="date">
                            ...
                            <span class="view-img"><a target="_blank" href="{{$mediaPath}}">View Document</a></span>
                        </div>
                        <!-- The Modal -->
